## analyze_real_mass.py
## ParT score threshold별 실제 데이터 m_mumu 분포를 정량 분석합니다.
##
## 계산 항목:
##   - selected events
##   - events in mass range
##   - peak position
##   - peak height
##   - FWHM
##   - signal window fraction
##
## 실행 예:
## python analysis_real_mass.py \
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        default="../processed/real_mass_summary_particle_transformer_v4.csv",
    )
...
def compute_fwhm(bin_centers, counts):
    """
    Compute FWHM from histogram counts.
...
def analyze_threshold(
    ...
    if len(mass) == 0 or np.max(counts) == 0:
        return {
            "threshold": threshold,
    ...
    mass_window_events = int(len(mass))
