## train_particle_transformer.py
## Particle Transformer 통합 학습 코드입니다.
##
## 실행 예:
## python train_particle_transformer.py --version v1 --epochs 5 --batch-size 4096 --lr 1e-4 --num-workers 4
## python train_particle_transformer.py --version v2 --epochs 5 --batch-size 4096 --lr 1e-4 --num-workers 4
## python train_particle_transformer.py --version v3 --epochs 5 --batch-size 4096 --lr 1e-4 --num-workers 4
## python train_particle_transformer.py --version v4 --epochs 5 --batch-size 4096 --lr 1e-4 --num-workers 4
...
PARTICLE_TRANSFORMER_CONFIGS = {
    "v1": {
        "model_class": ParticleTransformerV1,
        "checkpoint_name": "particle_transformer_v1_best.pt",
        "pairwise_features": ("delta_eta", "delta_phi"),
    },
    "v2": {
        "model_class": ParticleTransformerV2,
        "checkpoint_name": "particle_transformer_v2_best.pt",
        "pairwise_features": ("delta_eta", "delta_phi", "delta_r"),
    },
    "v3": {
        "model_class": ParticleTransformerV3,
        "checkpoint_name": "particle_transformer_v3_best.pt",
        "pairwise_features": (
            "delta_eta",
            "delta_phi",
            "delta_r",
        ...
        "v4": {
        "model_class": ParticleTransformerV4,
        "checkpoint_name": "particle_transformer_v4_best.pt",
        "pairwise_features": (
            "delta_eta",
            "delta_phi",
            "delta_r",
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="v1",
        choices=list(PARTICLE_TRANSFORMER_CONFIGS.keys()),
    )
...
def main():
    ...
    version_config = PARTICLE_TRANSFORMER_CONFIGS[args.version]
    model_class = version_config["model_class"]
    checkpoint_name = version_config["checkpoint_name"]
    pairwise_features = version_config["pairwise_features"]

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print("Device:", device)
    print("Model: ParticleTransformer", args.version.upper())
    print("Pairwise features:", pairwise_features)
