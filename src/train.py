## train.py는 학습 데이터를 이용하여 모델을 학습하는 코드입니다.
## 명령어는 python train.py --epochs <number_of_epochs> --batch-size <batch_size> --lr <learning_rate> --num-workers <number_of_workers> --output-dir <output_directory> 형태로 실행할 수 있습니다.
import argparse
import os
...
class BaselineMuonNet(nn.Module):
    def __init__(self, n_features=6, hidden_dim=128):
        super().__init__()

        self.muon_mlp = nn.Sequential(
    ...
    def forward(self, X, mask):
        ...
        logits = self.event_mlp(event).squeeze(-1)
        return logits
...
def main():
    ...
    model = BaselineMuonNet(n_features=6).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_loss < best_valid_loss:
            ...
            output_path = os.path.join(args.output_dir, "baseline_best.pt")
            torch.save(
                {
